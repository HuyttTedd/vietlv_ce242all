...
class Time extends AbstractElement
{
    ...
    public function __construct(
        ...
        $this->setType('mp_paymentrestriction_time');
    }
    ...
    public function getElementHtml()
    {
        ...
        $value_hrs = ($this->getName() == 'rule[schedule_name][to_time][]') ? 23 : 0;
        $value_min = ($this->getName() == 'rule[schedule_name][to_time][]') ? 59 : 0;
        ...
        for ($i = 0; $i < 24; $i++) {
            ...
            $html .= '<option value="' . $hour . '" '
                . ($value_hrs == $i ? 'selected="selected"' : '') . '>' . $hour . '</option>';
        }
        $html .= '</select>' . "\n";
        $html .= ':&nbsp;<select name="' . $this->getName() . '" style="width:80px" ' . $this->serialize(
            $this->getHtmlAttributes()
        ...
        for ($i = 0; $i < 60; $i++) {
            $hour = str_pad($i, 2, '0', STR_PAD_LEFT);
            $html .= '<option value="' . $hour . '" '
                . ($value_min == $i ? 'selected="selected"' : '') . '>' . $hour . '</option>';
        }
        $html .= '</select>' . "\n";
