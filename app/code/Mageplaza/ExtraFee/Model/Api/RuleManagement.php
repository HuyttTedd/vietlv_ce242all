...
class RuleManagement implements RuleInterface
{
    ...
    public function update($cartId, $area, ShippingInformationInterface $addressInformation)
    {
        ...
        // Can't use session on Rest API to collect totals
        if (!$this->isRest) {
    ...
    public function updateMultiShipping($cartId, $area, $shippingMethods)
    {
        ...
        foreach ($addresses as $address) {
            ...
            list($applyRule, $selectedOption) = $this->multiShippingExtraFee->getApplyRule($address, $area);
        ...
        if ((int) $area === DisplayArea::PAYMENT_METHOD
            ...
            list($applyRule, $selectedOption) = $this->multiShippingExtraFee->getApplyRule(
                $billingAddress,
                implode(',', $area)
            );
