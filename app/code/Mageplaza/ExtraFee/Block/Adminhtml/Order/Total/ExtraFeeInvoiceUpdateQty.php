...
use Mageplaza\ExtraFee\Block\Sales\Order\Invoice\Create\ExtraFee;

/**
 * Class ExtraFeeInvoiceUpdateQty
 * @package Mageplaza\ExtraFee\Block\Adminhtml\Order\Total
 */
class ExtraFeeInvoiceUpdateQty extends Items
{
    /**
     * @var ExtraFeeData
     */
    ...
    /**
     * ExtraFeeInvoiceUpdateQty constructor.
     *
        ...
        try {
            if ($this->extraFeeData->isEnabled()) {
                $html .= $this->getLayout()->createBlock(ExtraFee::class)
                    ->setTemplate('Mageplaza_ExtraFee::order/invoice/new/extra-fee.phtml')->toHtml();
            }
        } catch (Exception $e) {
            $html .= '';
        }
