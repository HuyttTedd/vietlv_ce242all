...
namespace  Mirasvit\Seo\Ui\CanonicalRewrite\Listing;
...
class DataProvider extends \Magento\Framework\View\Element\UiComponent\DataProvider\DataProvider
{
    ...
    public function getSearchResult()
    {
        ...
        foreach ($this->getSearchCriteria()->getFilterGroups() as $group) {
            ...
            foreach ($group->getFilters() as $filter) {
                if ($filter->getField() == CanonicalRewriteStoreInterface::STORE_ID) {
    ...
    {
        $query = 'SELECT ' . CanonicalRewriteStoreInterface::CANONICAL_REWRITE_ID . ' FROM '
            . $this->connection->getTableName(CanonicalRewriteStoreInterface::TABLE_NAME)
            . ' WHERE store_id IN (' . addslashes(implode(',', $fieldStoreValue)) . ')';

        $storeData = $this->connection->getConnection('read')->fetchAll($query);
        $linkIds = [];
        foreach ($storeData as $store) {
            $linkIds[] = $store[CanonicalRewriteStoreInterface::CANONICAL_REWRITE_ID];
        }
